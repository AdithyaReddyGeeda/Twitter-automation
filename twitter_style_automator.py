...
from config import DB_PATH, STYLE_PROFILE_PATH, X_HANDLE
from poster import (
    get_tweepy_api,
...
from style_analyzer import analyze_style, load_style_profile
...
def cmd_fetch_tweets(args: argparse.Namespace) -> int:
    """Fetch tweets for the configured handle and store in DB."""
...
def cmd_analyze_style(args: argparse.Namespace) -> int:
    """Analyze stored tweets and write style profile."""
    handle = args.handle or X_HANDLE
    try:
...
def cmd_generate_tweet(args: argparse.Namespace) -> int:
    """Generate one tweet, optionally for a topic."""
    topic = args.topic
    if not topic and args.suggest:
        suggestions = suggest_topics(args.profile)
...
def cmd_reply_mentions(args: argparse.Namespace) -> int:
    ...
    try:
        run_mentions_reply_job(reply_generator=None, dry_run=args.dry_run)
        return 0
    except Exception as e:
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    posting scheduler. Use this when you want one command that does everything.
    Optional: skip fetch/analyze with --no-fetch / --no-analyze.
    """
    from config import X_BEARER_TOKEN
    ...
    # 1. Fetch tweets if DB empty or stale (older than refresh_days)
    if not getattr(args, "no_fetch", False):
        try:
            tweets = get_all_tweets_from_db(db_path=db_path, limit=1)
            ...
            need_fetch = not tweets or (
                refresh_days > 0 and (datetime.utcnow().timestamp() - db_mtime) > refresh_days * 86400
            )
            if need_fetch:
    ...
    # 2. Analyze style if profile missing or older than DB
    if not getattr(args, "no_analyze", False):
        try:
            profile_path_p = Path(profile_path)
            db_path_p = Path(db_path)
            profile_exists = profile_path_p.exists()
            db_newer = (
                db_path_p.exists() and profile_path_p.exists()
                and db_path_p.stat().st_mtime > profile_path_p.stat().st_mtime
            )
    ...
    # 3. Run the posting scheduler (blocks until Ctrl+C)
...
def main() -> int:
    ...
    # fetch-tweets
    p_fetch = subparsers.add_parser("fetch-tweets", help="Fetch user timeline into DB")
    ...
    # analyze-style
    p_analyze = subparsers.add_parser("analyze-style", help="Analyze tweets and build style profile")
